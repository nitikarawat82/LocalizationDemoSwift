...
//now to align text of all the texfield throughout the app when language code is ar

extension UITextField {
    open override func awakeFromNib() {
        super.awakeFromNib()
        NotificationCenter.default.addObserver(self, selector: #selector(languageChanged), name: NSNotification.Name(rawValue: "LanguageChangedNotification"), object: nil)
        updateTextAlignment()
    }
    ...
    private func updateTextAlignment() {
        guard LocalizationSystem.sharedInstance.getLanguage() == "ar", textAlignment == .natural else { return }
        textAlignment = .right
    }
}
